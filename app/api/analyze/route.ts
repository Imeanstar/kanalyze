...
반드시 포함할 내용: 핵심 성격 분석(대화 샘플 발췌 인용 포함), 그리고 가장 중요한 "핵심 관계도" ASCII 아트.

[작성 예시]
(자연스러운 도입부 및 2~3줄 분량의 성격/역할 요약)
...
중요: 위 예시처럼 선과 기호( / | \\ - )를 이용한 디테일한 ASCII 아트 관계도를 반드시 포함하고, 띄어쓰기 렌더링이 깨지지 않도록 ASCII 아트 부분은 반드시 마크다운의 \`\`\`text ... \`\`\` 블록으로 감싸주세요! "총 메시지 수"나 "기본 프로필" 같은 통계성 텍스트는 UI에 표기되므로 절대로 출력 내용에 포함하지 마세요. 바로 본론(성격 분석)으로 들어가주세요.
...
아래 JSON 형식으로만 응답하세요 (설명 없이 JSON만. 코드 블록(\`\`\`json) 기호도 절대 쓰지 말고 오직 순수 JSON 텍스트 상태로 반환하세요. 문자열 내부의 따옴표나 줄바꿈은 반드시 JSON 규칙에 맞게 이스케이프 처리하세요):
{
  ...
  "detailed_markdown": "위에서 요구한 성격 분석 보고서와 ASCII 관계도 코드블록이 모두 포함된 텍스트 전장 (줄바꿈은 \\n 으로 이스케이프 처리할 것)"
}`;
}
